...
bandsInTownRouter.post("/saveEvent", async (req, res) => {
  try {
    ...
    let checkQuery =
      "SELECT * FROM SavedEvents WHERE spotifyID = ? AND artistName = ? AND dateTime = ?";
    let checkParams = [spotifyID, artistName, dateTime];
    let result = await db.get(checkQuery, checkParams);
    if (result) {
      res.status(409).json({ message: "Event already exists" });
      ...
      let query =
        "INSERT INTO SavedEvents (spotifyID, artistName, venue, dateTime, lineup, location, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
      let params = [
        spotifyID,
        artistName,
        venue,
        dateTime,
        JSON.parse(lineup).join(", "),
        location,
        latitude,
        longitude,
      ];
